...
def einfo_crawler(url):
    ...
    for story_link in story_links:
        try:
            story_text_link = "http://e-info.org.tw" + story_link.get("href")
        except:
            continue
        try:
...
def get_text(url, story_title):
    ...
    for x in tree.find('.//div[@class="bodycontent"]').iter():
        try:
            if x.tag == "p":
                t = x.text.strip()
...
if __name__ == "__main__":
    # print get_text("http://e-info.org.tw/node/109264")
    einfo_crawler(url="http://e-info.org.tw/taxonomy/term/13")