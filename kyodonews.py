...
def kyodonews_crawler(url):
    text = get_page(url)
    parser = etree.HTMLParser()
    tree = etree.parse(StringIO(text), parser)

    story_links = tree.xpath('.//div[@id="contentsArea"]//a')

    for story_link in story_links:
        try:
            story_text_link = "http://china.kyodonews.jp" + story_link.get("href")
        except:
            continue
        try:
...
if __name__ == "__main__":
    kyodonews_crawler(url="http://china.kyodonews.jp/news/japan-china_relationship/")