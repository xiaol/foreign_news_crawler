...
def udn_crawler(url):
    ...
    story_links = tree.xpath('.//div[@id="mainbar"]//a')

    for story_link in story_links:
        try:
            story_text_link = "http://udn.com/news" + story_link.get("href")
        except:
            continue
        try:
...
def get_text(url, story_title):
    ...
    for x in tree.find('.//div[@id="story_body_content"]').iter():
        try:
            if x.tag == "p":
                t = x.text.strip()
...
if __name__ == "__main__":
    udn_crawler(url="http://udn.com/news/cate/6640")
    udn_crawler(url="http://udn.com/news/cate/6643")