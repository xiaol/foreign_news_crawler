...
def stgloballink_crawler(url):
    text = get_page(url)
    parser = etree.HTMLParser()
    tree = etree.parse(StringIO(text), parser)

    story_links = tree.xpath('.//div[@class="cnt"]//a')
...
def get_text(url, story_title):
    ...
    for x in tree.find('.//div[@class="cnt"]').iter():
        try:
            if x.tag == "p":
                t = x.text.strip()
                if len(t) != 0:
...
if __name__ == "__main__":
    stgloballink_crawler(url="http://ny.stgloballink.com/usa/")
    stgloballink_crawler(url="http://ny.stgloballink.com/china/")