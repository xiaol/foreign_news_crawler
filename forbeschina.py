...
def forbeschina_crawler(url):
    text = get_page(url)
    parser = etree.HTMLParser()
    tree = etree.parse(StringIO(text), parser)

    story_links = tree.xpath('.//div[@id="main_content"]//a')

    for story_link in story_links:
        try:
            story_text_link = "http://www.forbeschina.com" + story_link.get("href")
        except:
            continue
        try:
...
if __name__ == "__main__":
    forbeschina_crawler(url="http://www.forbeschina.com/news/news_hot.php")